...
python FixedWidthToDelimiter.py -i SrcFile.txt -o TrgFcd usrile.txt -c Config.txt -d "|"
python FixedWidthToDelimiter.py -i SrcFile.txt -o TrgFile.txt -c Config.txt -d ","

Inputs are as follows
1. Input File - Mandatory(Argument -i) - File which has fixed Width data in it
2. Output File - Optional (Argument -o, if not provided will be used as InputFIleName plus Delimited.txt)
3. Config File - Optional (Argument -c, if not provided will look for Config.txt file on same path, if not present script will not run)
